...
{
	struct filehandle_t *fh = &(file_handle_array[file_handle]);
	if (fh->inode_number<0) return -1;
	//fetch that inode number
    struct inode_t *inode = (struct inode_t *)malloc(sizeof(struct inode_t));
	simplefs_readInode(fh->inode_number, inode);
	if (fh->offset+nbytes>inode->file_size) return -1;
	int curbyte=fh->offset;
	while (curbyte<fh->offset+nbytes)
	{
		//fetch the current block
		char curblock[BLOCKSIZE];
		int curblocknum=curblocknum/BLOCKSIZE;
		simplefs_readDataBlock(inode->direct_blocks[curblocknum], curblock);
		while (curbyte<fh->offset+nbytes && curbyte!=(curblocknum+1)*BLOCKSIZE)
		{
			buf[curbyte-fh->offset]=curblock[curbyte%BLOCKSIZE];
			curbyte++;
		}
	}
	return 0;
