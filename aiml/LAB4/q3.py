...
class  NaiveBayes:
	...
	def _calc_class_prior(self):
		...
		for outcome in np.unique(self.y_train):
			...
			#TODO_1
			self.class_priors[outcome] = len(self.y_train[self.y_train == outcome])/len(self.y_train)
    		### YOUR CODE ENDS HERE ###
	...
	def predict(self, X):
		...
		for query in X:
			...
			for outcome in np.unique(self.y_train):
				...
				posterior = likelihood*prior/evidence #TODO_2         Remove 0 and get posterior

				probs_outcome[outcome] = posterior
			
			result = sorted(probs_outcome.items(), key=lambda x:-x[1])[0][0] #TODO_3          Remove None and get the Max
