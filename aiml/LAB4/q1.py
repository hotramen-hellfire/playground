...
def cost_logreg(X, y, w, w_0):
    ...
    ### YOUR CODE BEGINS HERE ###
    # print("cost_logreg")
    y_dash = logistic(np.matmul(X,w)+w_0)
    # for i in range(len(y_dash)) : 
    #     if y_dash[i]==0 : y_dash[i]=0.0000001 
    cost = np.mean(log_loss(y, y_dash))
    ### YOUR CODE ENDS HERE ###
...
def grad_logreg(X, y, w, w_0):
    ...
    ### YOUR CODE BEGINS HERE ###
    ##calculate Xw+w_0 before hand saves multiplicatio
    # print("grad_logreg")
    c = logistic(np.matmul(X,w)+w_0)-y
    grad_w = (1/X.shape[0])*np.matmul(X.T , c)
    # print(X.shape,w.shape,grad_w.shape)
    grad_w_0 = np.mean(c)
    ### YOUR CODE ENDS HERE ###
