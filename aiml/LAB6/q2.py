...
class SKLearnDecisionTree:
    ...
    def train_model(self, criterion="entropy"):
        ...
        ### Student Code start
        results = {}
        self.tree = DecisionTreeClassifier(criterion=criterion, random_state=42)
        self.tree.fit(self.X_train, self.Y_train)
        results['train_accuracy']=metrics.accuracy_score(self.Y_train, self.tree.predict(self.X_train))
        results['test_accuracy']=metrics.accuracy_score(self.Y_test, self.tree.predict(self.X_test))
        return results
        ### Student Code end
    ...
    def train_model_regularised(self, criterion="entropy"):
        ...
        ### Student Code start
        samples_depths = [i for i in range(1,11)]
        samples_split = [i for i in range(3,31)]
        samples_split = samples_split[::3]
        samples_leaf = [i for i in range(1,11)]
        for depth in samples_depths :
            for split in samples_split :
                for leaf in samples_leaf :
                    self.tree = DecisionTreeClassifier(criterion=criterion, random_state=42, min_samples_leaf=leaf, max_depth=depth, min_samples_split=split)
                    self.tree.fit(self.X_train, self.Y_train)
                    train_accuracies.append(metrics.accuracy_score(self.Y_train, self.tree.predict(self.X_train)))
                    test_accuracies.append(metrics.accuracy_score(self.Y_test, self.tree.predict(self.X_test)))
