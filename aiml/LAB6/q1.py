...
def gini_index(classes):
    ...
    uniqs=np.array(np.unique(classes, return_counts=True), dtype=np.float64)
    totale=len(classes)
    # Calculate the probabilities of each unique label occurring
    uniqs[1]=uniqs[1]/totale
    # Calculate the gini index value using the formula: 1 - Σ(p_i^2)
    uniqs[1]=uniqs[1]**2
    gini_impurity_value=1-np.sum(uniqs[1])
...
def entropy(classes):
    ...
    uniqs[1]+=10**(-12)
    logEUniqs1=np.log(uniqs[1])/np.log(2)
    entropy_value=-np.dot(uniqs[1], logEUniqs1)
...
def information_gain(examples, classes, attr):
    ...
    uniqs=np.array(np.unique(np.array(examples[:,attr]), return_counts=True, return_index=False))
    # Calculate the entropy of the entire dataset before splitting
    E_before = entropy(classes)
    num_classes=len(examples)
    ## Calculate the entropy after splitting for all unique values of the attribute
    E_after = 0
    freqs = np.array(uniqs[1], dtype=np.float64)
    for i in range(len(uniqs[0])) :
        indices = np.where(examples[:,attr]==uniqs[0][i])[0]
        E_after+=(freqs[i]/num_classes)*entropy(classes[indices])
    information_gain_value = E_before-E_after
...
def gini_gain(example, classes, attr):
    ...
    # Get unique values and their counts for the specified attribute
    uniqs=np.array(np.unique(np.array(example[:,attr]), return_counts=True, return_index=False))
    # Calculate the gini index of the entire dataset before splitting
    G_before=gini_index(classes)
    num_classes=len(example)
    ...
    freqs = np.array(uniqs[1], dtype=np.float64)
    for i in range(len(uniqs[0])) :
        indices = np.where(example[:,attr]==uniqs[0][i])[0]
        G_after+=(freqs[i]/num_classes)*gini_index(classes[indices])
    gini_gain_value = G_before-G_after
...
class DecisionTree:
    ...
    def __build_tree__(self, features, classes, attributes, depth=0):
        ...
        if (len(attributes) == 0):
            ...
            information_gains = np.array([information_gain(examples=features, classes=classes, attr=attr) for attr in attributes])
            index = np.argmax(information_gains)
            best_attr = attributes[np.argmax(information_gains)]
            # Get unique values of the best attribute for splitting.
            uniqs=np.array(np.unique(np.array(features[:,best_attr]), return_counts=True))
            ...
            for i in range(len(uniqs[0])) :
                indices = np.where(features[:,best_attr]==uniqs[0][i])[0]
                if (len(indices)>0) :
                    features_new = features[indices]
                    classes_new = classes[indices]
                    children[uniqs[0][i]]=self.__build_tree__(features=features_new, classes=classes_new, attributes=attributes, depth=depth+1)
                else :
                    if (classes.sum()*2 >= classes.size):
                        children[uniqs[0][i]]=DecisionNode(1)
                    else:
                        children[uniqs[0][i]]=DecisionNode(0)
