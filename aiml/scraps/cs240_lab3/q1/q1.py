...
def create_weights(data_dictionary, lambda_val):
    ...
    weights =  np.matmul(np.matmul(np.linalg.inv(np.matmul(X.T, X) + lambda_val*np.eye(X.shape[1])), X.T), Y)
    # END TODO
...
def generate_test_error(data_dictionary, weights):
    ...
    # print(X.shape, weights.shape, Y.shape, np.matmul(X, weights).shape)
    test_error = np.linalg.norm(Y-np.matmul(X, weights), ord=2)/Y.shape[0]
    # test_error = np.sum([(yi-np.matmul(xi,weights)[0])**2 for yi, xi in zip(Y, X)])/Y.shape[0]
    # print(test_error)
    # loss = (1/Y.shape[0])*sum([(yi - np.matmul(w.t(), xi))**2 for yi, xi in zip(Y, X)])
    # loss = loss.item()
    # END TODO
...
def find_optimal_lambda(error_array, lambda_array):
    ...
    grid = np.array([i for i,j in zip(error_array, lambda_array)])
    optimal_lambda = lambda_array[np.where(grid == grid.min())[0][0]]
    # END TODO
