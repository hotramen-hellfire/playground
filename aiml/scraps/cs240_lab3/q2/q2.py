...
def lasso_loss(X, Y, w, lambd):
    ...
    loss = (1/Y.shape[0])*(torch.linalg.vector_norm(Y-torch.matmul(X, w), ord=2))**2 + lambd*torch.linalg.vector_norm(w, ord=1)
    ### YOUR CODE ENDS HERE ###
...
def lasso_loss_derivative(X, Y, w, lambd):
    ...
    derivative = lambd*torch.sgn(w) + (2/Y.shape[0])*torch.matmul(X.t(), torch.matmul(X, w)-Y)
    ### YOUR CODE ENDS HERE ###
