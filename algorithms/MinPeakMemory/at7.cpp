...
int evaluatesequence(vector<int> &sequence, vector<Ks> &oldK, int proc, int posn, const vector<vector<int>> &memory, vector<int> &newseq, vector<Ks> &newK)
{
    ...
    if (posn != 0)
    {
        t2 = newK[posn - 1].forwd;
        oh1 = newK[posn - 1].ovhd;
    };
    if (posn != newseq.size() - 1)
    {
        t3 = newK[posn + 1].forwd;
        oh2 = newK[posn + 1].ovhd;
    };
    ...
    for (int i = 0; i < newseq.size(); i++)
        peak_memory = max(peak_memory, newK[i].forwd + newK[i].ovhd + newK[i].back + newK[i].curr);
    return peak_memory;
}
...
int givesetsize(long long num, int n)
{
    int ret = 0;
    for (int i = n - 1; i >= 0; i--)
    {
