...
int main()
{
    ...
    vector<vector<vector<int>>> sequence((1 << n), vector<vector<int>>{{}});
    peaks[0] = 0;
    for (long long ss = 0; ss <= (1 << n) - 1; ss++)
    {
        ...
        for (auto seqss : sequence[ss])
        {
            ...
            for (int newel = maxh(ss, n); newel < n; newel++) /// scope
            {
                ...
                {
                    vector<int> prevseq = seqss;
                    int bestvalue = peaks[((1 << newel) | ss)];
                    vector<int> bestposns;
                    ...
                    if (bestvalue< peaks[((1 << newel) | ss)])
                    {
                        ...
                        for (auto pos: bestposns)
                        {
                            vector<int> newseq = prevseq;
                            newseq.insert(newseq.begin() + pos, newel);
                            sequence[((1 << newel) | ss)].push_back(newseq);
                        }
                    }
